...
class Recommender():
    ...
    def __init__(self, id=None,
                 timeInterval=(datetime.datetime.strptime('00:00', '%H:%M').time(),
    ...
    def __sortModules(self, modules):
        order = sorted(range(len(modules)),
                       key=lambda i: (modules[i].selections, modules[i].title))  # sort according to choices?!?!?!
    ...
    def updateFilters(self, timeInterval=None, credits=None, exam_types=None, locations=None, categories=None,
                      interests=None):
    ...
    def get_recommendation_from_json(json_object):
        ...
        if 'filters' in cleanup_json_object:
            ...
            if 'time' in filters:
                ...
                format = '%H:%M'
                timeInterval = (datetime.datetime.strptime(filters['time'][0], format).time(),
