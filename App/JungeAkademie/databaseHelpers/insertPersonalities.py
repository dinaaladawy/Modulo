...
django.setup()
from modulo.models import Personality
...
def insertPersonalities(databaseFile):
    ...
    for row in worksheet.iter_rows(min_row=2, min_col=1, max_row=nrRows, max_col=nrCols):
        ...
        if cell.value is None:
            continue
        i, added = Personality.objects.get_or_create(personality=cell.value.strip())
        add += 1 if added else 0

    print("Number of added personalities =", add)
    print('Number of personalities in database =', len(Personality.objects.all()))
    #workbook.close()
